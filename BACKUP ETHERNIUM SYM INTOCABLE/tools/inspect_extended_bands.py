...
img = cv2.imread("tools/hq_extended_inspect.png", cv2.IMREAD_GRAYSCALE)
if img is not None:
    _, thresh = cv2.threshold(img, 30, 255, cv2.THRESH_BINARY)
    proj = np.sum(thresh > 0, axis=1)
    
    # find all active y ranges
    active = proj > 5
    bands = []
    start = None
    ...
    print("Ink bands detected in hq_extended_inspect.png (relative to y=2000):")
